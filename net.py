...
class Unet1(nn.Module):
    def __init__(self, stride=1):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(1,64,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.pool1 = nn.MaxPool2d(kernel_size = 2,stride=2)
        self.conv2 = nn.Sequential(nn.Conv2d(64,128,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.pool2 = nn.MaxPool2d(kernel_size = 2,stride=2)
        self.conv3 = nn.Sequential(nn.Conv2d(128,256,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.pool3 = nn.MaxPool2d(kernel_size = 2,stride=2)
        self.conv4 = nn.Sequential(nn.Conv2d(256,512,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.pool4 = nn.MaxPool2d(kernel_size = 2,stride=2)
        
        self.conv5 = nn.Sequential(nn.Conv2d(512,256,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.up1 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv6 = nn.Sequential(nn.Conv2d(512,128,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.up2 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv7 = nn.Sequential(nn.Conv2d(256,64,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.up3 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv8 = nn.Sequential(nn.Conv2d(128,64,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.conv9 = nn.Sequential(nn.Conv2d(64,1,kernel_size=3,stride=1,padding=1))
    ...
    def forward(self,input):
        ...
        out9=self.conv5(out7)
        out10=self.up1(out9)
        out10=torch.cat([out10,out5],dim=1)
        out11=self.conv6(out10)
        out12=self.up2(out11)
        out12=torch.cat([out12,out3],dim=1)
        out13=self.conv7(out12)
        out14=self.up3(out13)
        out14=torch.cat([out14,out1],dim=1)
        out15=self.conv8(out14)
        out16=self.conv9(out15)
...
class Unet2(nn.Module):
    def __init__(self, stride=1):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(1,64,kernel_size=7,stride=2,padding=3),nn.ReLU(True))
        self.conv2 = nn.Sequential(nn.Conv2d(64,128,kernel_size=5,stride=2,padding=2),nn.ReLU(True))
        self.conv3 = nn.Sequential(nn.Conv2d(128,256,kernel_size=5,stride=2,padding=2),nn.ReLU(True))
        self.conv4 = nn.Sequential(nn.Conv2d(256,512,kernel_size=3,stride=2,padding=1),nn.ReLU(True))
        
        self.up1 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv5 = nn.Sequential(nn.Conv2d(512+256,256,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.up2 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv6 = nn.Sequential(nn.Conv2d(256+128,128,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.up3 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv7 = nn.Sequential(nn.Conv2d(128+64,64,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.up4 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv8 = nn.Sequential(nn.Conv2d(64+1,64,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.conv9 = nn.Sequential(nn.Conv2d(64,64,kernel_size=3,stride=1,padding=1),nn.ReLU(True))
        self.conv10 = nn.Sequential(nn.Conv2d(64,1,kernel_size=1,stride=1,padding=0),nn.ReLU(True))
    ...
    def forward(self,input):
        ...
        out5=self.up1(out4)
        out5=torch.cat([out5,out3],dim=1)
        out6=self.conv5(out5)
        out7=self.up2(out6)
        out7=torch.cat([out7,out2],dim=1)
        out8=self.conv6(out7)
        out9=self.up3(out8)
        out9=torch.cat([out9,out1],dim=1)
        out10=self.conv7(out9)
        out11=self.up4(out10)
        out11=torch.cat([out11,input],dim=1)
        out12=self.conv8(out11)
        out13=self.conv9(out12)
        out14=self.conv10(out13)
...
class Unet3(nn.Module):
    def __init__(self, stride=1):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(1,64,kernel_size=3,stride=1,padding=1),nn.ReLU(True),nn.MaxPool2d(kernel_size=2,stride=2))
        self.conv2 = nn.Sequential(nn.Conv2d(64,128,kernel_size=3,stride=1,padding=1),nn.ReLU(True),nn.MaxPool2d(kernel_size=2,stride=2))
        self.conv3 = nn.Sequential(nn.Conv2d(128,256,kernel_size=3,stride=1,padding=1),nn.ReLU(True),nn.MaxPool2d(kernel_size=2,stride=2))
        self.conv4 = nn.Sequential(nn.Conv2d(256,512,kernel_size=3,stride=1,padding=1),nn.ReLU(True),nn.MaxPool2d(kernel_size=2,stride=2))
